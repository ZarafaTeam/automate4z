...
function loadConfig(): Config {
    ...
    try {
        ...
        console.error(`⚠️ Erreur lors de la lecture du fichier de configuration :`, err);
        process.exit(1);
    }
}
...
function saveConfig(config: Config) {
    try {
        ...
        console.error(`⚠️ Erreur lors de l'écriture du fichier de configuration :`, err);
        process.exit(1);
    }
}
...
async function addConnection(config: Config) {
  const { name, type } = await enquirer.prompt<{ name: string; type: string }>([
    { type: 'input', name: 'name', message: 'Nom de la configuration (ex: SDEV)' },
    {
      type: 'select',
      name: 'type',
  ...
  const entry: ConfigEntry = { name: name.toUpperCase(), type: type as 'zowe' | 'basic' };
...
async function removeConnection(config: Config) {
  ...
  await keytar.deletePassword("automate4z", `${name.toUpperCase()}_USER`).catch((err) => {console.log('Erreur lors de la suppression de l\'utilisateur :', err)});
  await keytar.deletePassword("automate4z", `${name.toUpperCase()}_PWD`).catch((err) => {console.log('Erreur lors de la suppression du mot de passe :', err)}); 
}
...
async function editConnection(config: Config) {
  ...
  const { newName, newType } = await enquirer.prompt<{ newName: string; newType: string }>([
    {
      type: 'input',
      name: 'newName',
  ...
  const updated: ConfigEntry = { name: newName, type: newType as 'zowe' | 'basic' };
  ...
  if (newType === 'zowe') {
    ...
    const user = await keytar.getPassword("automate4z", `${updated.name.toUpperCase()}_USER`);
    const answers = await enquirer.prompt<{
      hostname: string;
      port: number;
      user: string;
      ...
      { type: 'input', name: 'hostname', message: 'Hostname', initial: existing.hostname },
      { type: 'numeral', name: 'port', message: 'Port', initial: existing.port },
      { type: 'input', name: 'user', message: 'Utilisateur', initial: user },
      { type: 'password', name: 'password', message: 'Mot de passe (laisser vide pour ne pas changer si déjà existant)' }
    ]);

    const password = await keytar.getPassword("automate4z", `${updated.name.toUpperCase()}_PWD`) || '';
  ...
  if (updated.type === 'basic') {
    await keytar.setPassword("automate4z", `${updated.name.toUpperCase()}_USER`, updated.user ?? '');
    if (updated.password) {
      await keytar.setPassword("automate4z", `${updated.name.toUpperCase()}_PWD`, updated.password ?? '');
    }
  }
  ...
  if (name !== updated.name || (updated.type === 'zowe' && existing.type === 'basic')) {
    await keytar.deletePassword("automate4z", `${name.toUpperCase()}_USER`).catch((err) => {console.log('Erreur lors de la suppression de l\'utilisateur :', err)});
    await keytar.deletePassword("automate4z", `${name.toUpperCase()}_PWD`).catch((err) => {console.log('Erreur lors de la suppression du mot de passe :', err)}); 
  }
