...
describe("WorkflowRunner", () => {
    ...
    describe("runWorkflow", () => {
        it("devrait exécuter un workflow avec succès", async () => {
            const result = await runWorkflow("test.yaml", {});
        ...
        it("devrait injecter les variables d'environnement", async () => {
            ...
            const result = await runWorkflow("test.yaml", env);
            expect(result.success).toBe(true);
            expect(mockStepHandler.run).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
        ...
        it("devrait gérer les erreurs de lecture de fichier", async () => {
            ...
            const mockConsoleError = vi
                .spyOn(console, "error")
                .mockImplementation(() => { });
            await expect(runWorkflow("test.yaml", {})).rejects.toThrow();
            expect(mockConsoleError).toHaveBeenCalled();
            mockConsoleError.mockRestore();
        });
